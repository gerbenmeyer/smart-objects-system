 ...
 * Agent}</code>
 * with support functionality for TAC SCM. <code>SCMAgent</code> provides the
 * following features for the agent developer:
 * 
 * <ul>
 * <li> APIs for accessing game information.
 * <li> Automatic bookkeeping of RFQs, orders, and inventory.
 * <li> Support for creating production schedules based on inventory and
 * capacity.
...
public abstract class SCMAgent extends Agent {
	...
	private static final Logger log = Logger
			.getLogger(SCMAgent.class.getName());
	...
	private void checkInitialized() {
		if (!isInitialized && this.bomBundle != null && this.catalog != null
				&& this.startInfo != null && this.factoryAddress != null) {
			isInitialized = true;
			simulationStarted();
		}
	}
	...
	/**
	 * Called when a message has been received. This method should not be
	 * overridden.
	 * 
	 * @param message
	...
	protected void messageReceived(Message message) {
		...
		if (!isInitialized) {
			if (content instanceof ComponentCatalog) {
				...
				this.daysBeforeVoid = startInfo.getAttributeAsInt(
						"customer.daysBeforeVoid", this.daysBeforeVoid);
				this.factoryAddress = startInfo.getAttribute("factory.address");
				this.factoryCapacity = startInfo.getAttributeAsInt(
						"factory.capacity", 2000);
				...
				for (int i = 0, n = activeOrders.getCustomerOrderCount(); i < n; i++) {
					...
					if (customerOrders.getOrder(orderID) == null) {
						...
						Order order = new Order(activeOrders
								.getCustomerAddress(i), orderID, activeOrders
								.getCustomerProductID(i), activeOrders
								.getCustomerQuantity(i), activeOrders
								.getCustomerUnitPrice(i), activeOrders
								.getCustomerDueDate(i), activeOrders
								.getCustomerPenalty(i));
				...
				for (int i = 0, n = activeOrders.getSupplierOrderCount(); i < n; i++) {
					...
					if (supplierOrders.getOrder(orderID) == null) {
						// The order was not already known
						Order order = new Order(activeOrders
								.getSupplierAddress(i), orderID, activeOrders
								.getSupplierProductID(i), activeOrders
								.getSupplierQuantity(i), activeOrders
								.getSupplierUnitPrice(i), activeOrders
								.getSupplierDueDate(i), activeOrders
								.getSupplierPenalty(i));
			...
			synchronized (lock) {
				...
				if (supplierDeliveries.size() > 0) {
					for (int i = 0, n = supplierDeliveries.size(); i < n; i++) {
						DeliveryNotice notice = (DeliveryNotice) supplierDeliveries
								.get(i);

						for (int j = 0, m = notice.size(); j < m; j++) {
							nextDayInventory.addInventory(notice
									.getProductID(j), notice.getQuantity(j));
						}
					}
					supplierDeliveries.clear();
				}
				if (lastDayProduction != null) {
					// Add the production for the current day to the inventory
					// for
					// tomorrow
					for (int i = 0, n = lastDayProduction.size(); i < n; i++) {
						nextDayInventory.addInventory(lastDayProduction
								.getProductID(i), lastDayProduction
								.getQuantity(i));
					}
			...
			log.warning("ignoring unhandled message '"
					+ content.getTransportName() + '\'');
		}
	}
	...
	/**
	 * Returns the last received bank status. The bank status is received each
	 * day before the simulation status.
	 */
	...
	/**
	 * Returns the last received inventory. The inventory is received each day
	 * before the simulation status.
	 */
	 ...
	 * 
	 * The inventory for next day includes components that will be available
	 * next day (not in the this day's inventory) and also the products that are
	 * produced this day. It is used to bookkeep the production and deliveries
	 * for next day to ease the generation of delivery and production schedules.
	 */
	...
	protected void reserveInventoryForNextDay(int productID, int quantity) {
		...
		synchronized (lock) {
			int existingInventory = nextDayInventory
	...
	/**
	 * Returns the calculated free factory capacity after removing the capacity
	...
	/**
	 * Returns the customer order store. Information about the active customer
	 * orders (not yet delivered) can be found in the customer order store.
	 */
	 ...
	 * 
	 * <p>
	 * Note: to send the offers to the customers, the
	 * <code>sendCustomerOffers</code> method must be called after all offers
	 * been added. This must be done before the day ends!
	 * 
	 * @param rfqBundle
	 *            the RFQ bundle from the customers
	 * @param rfqIndex
	 *            the index of the RFQ to bid on
	 * @param offeredUnitPrice
	...
	protected int addCustomerOffer(RFQBundle rfqBundle, int rfqIndex,
		...
		synchronized (lock) {
			...
			Order offer = new Order(customerAddress, offerID, rfqBundle,
					rfqIndex, offeredUnitPrice);
			// add offer
	 ...
	 * 
	 * @param orders
	 *            the received orders from the customers
		...
		synchronized (lock) {
			...
			for (int i = 0, n = orders.size(); i < n; i++) {
				...
				if ((offerID >= customerOffers.size())
						|| ((offer = (Order) customerOffers.get(offerID))
								.isOrdered())) {
			...
			if (newOrderCount < newOrders.length) {
				newOrders = (Order[]) ArrayUtils.setSize(newOrders,
						newOrderCount);
			}
			return newOrders;
		}
	 ...
	 * 
	 * <p>
	 * Note: to send the RFQs to the suppliers, the
	 * <code>sendSupplierRFQs</code> method must be called after all RFQs been
	 * added. This must be done before the day ends!
	 * 
	 ...
	 *             <code>addSupplierRFQ(String,int,int,int,int)</code>
	 */
	protected int addSupplierRFQ(String supplierAddress, int productID,
	 ...
	 */
	protected int addSupplierRFQ(String supplierAddress, int productID,
			int quantity, int reservePricePerUnit, int dueDate) {
		synchronized (lock) {
			int rfqID = supplierRFQs.addRFQ(supplierAddress, productID,
					quantity, dueDate, currentDate);

			RFQBundle bundle = (RFQBundle) supplierRFQTable
					.get(supplierAddress);
			if (bundle == null) {
				bundle = new RFQBundle();
				supplierRFQTable.put(supplierAddress, bundle);
			}
			bundle.addRFQ(rfqID, productID, quantity, reservePricePerUnit,
					dueDate, 0);
			return rfqID;
		}
	}
	 ...
	 * 
	 * <p>
	 * Note: to send the orders to the suppliers, the
	 * <code>sendSupplierOrders</code> method must be called after all orders
	 * been added. This must be done before the day ends!
	 * 
	 * @param supplierAddress
	 *            the supplier to order from
	 * @param offers
	 *            the offer bundle from the supplier
	 * @param offerIndex
	 *            the index of the offer in the bundle
	...
	protected Order addSupplierOrder(String supplierAddress,
		...
		synchronized (lock) {
			...
			Order order = supplierOrders.createOrder(supplierAddress, offers,
					offerIndex, productID);
			int orderID = order.getOrderID();

			OrderBundle bundle = (OrderBundle) supplierOrderTable
					.get(supplierAddress);
			if (bundle == null) {
	...
	protected void sendSupplierOrders() {
		if (supplierOrderTable.size() > 0) {
			synchronized (supplierOrderTable) {
				...
				while (keyIterator.hasMoreElements()) {
					String supplierAddress = (String) keyIterator.nextElement();
					OrderBundle orderBundle = (OrderBundle) supplierOrderTable
							.get(supplierAddress);
	 ...
	 * possible.
	 * 
	 * This method is using the calculated inventory for next day to determine
	 * if the delivery can be made or not. If it can be made the delivered
	 * products are removed from the inventory status and the order is marked as
	 * delivered.
	 * 
	 * <p>
	 * Note: the <code>sendFactorySchedules</code> method must be used to send
	 * the delivery schedule to the factory after all deliveries have been
	 * added. This must be done before the day ends!
	 * 
	 * @param order
	 *            the order to deliver for.
	...
	protected boolean addDeliveryRequest(Order order) {
		synchronized (lock) {
			...
			schedule.addDelivery(productID, quantity, order.getOrderID(), order
					.getOtherAgent());
	...
	/**
	 * Adds a production request to the production schedule for next day if
	 * possible.
	 * 
	 * This method is using the calculated inventory for next day to determine
	 * if the production can be made or not. If it can be made the components
	 * used for the production are removed from the inventory status.
	 * 
	 * <p>
	 * Note: the <code>sendFactorySchedules</code> method must be used to send
	 * the production schedule to the factory after all production requests have
	 * been added. This must be done before the day ends!
	 * 
	 * @param productID
	 ...
	 * @return true if it is possible to produce the specified products and
	 *         false otherwise.
	 * @see #sendFactorySchedules
	 */
	protected boolean addProductionRequest(int productID, int quantity) {
		...
		if (cyclesReq <= 0) {
			throw new IllegalArgumentException(
					"no assembly cycle specification " + "for product "
							+ productID + " was found in the BOM");
			// return false;
		}
		int[] components = bomBundle.getComponents(index);
		if (components == null) {
			throw new IllegalArgumentException("no components "
					+ "for product " + productID + " was found in the BOM");
			// return false;
		}
			...
			if (productionSchedule != null) {
				if (lastDayProduction != null) {
					throw new IllegalStateException(
							"schedules for next day already "
									+ "sent to factory");
				}
	 ...
	 * TAC SCM the customers only send one bundle per day and the same RFQs are
	 * sent to all manufacturers.
	 * 
	 * @param rfqBundle
	 *            a bundle of RFQs
	 */
	 ...
	 * TAC SCM the customers only send one order bundle per day as response to
	...
	/**
	 * Called when a bundle of offers have been received from a supplier. In TAC
	 * SCM suppliers only send on offer bundle per day in reply to RFQs (and
	 * only if they had something to offer).
	 * 
	 * @param supplierAddress
	...
	/**
	 * Called when a delivery notice has been received from a supplier. In TAC
	 * SCM only the suppliers send delivery notices to manufacturers. A delivery
	 * notice contains the components that will be delivered during the day and
	 * these components will be available for production the next day. The
	 * supplier address can be determined with the <code>getSupplier</code>
	 * method in the delivery notice.
	 * 
	 * @param notice
	 *            a delivery notice with a list of components that are about to
	 *            be delivered
	 ...
	 * 
	 * @param status
	 *            the latest factory status (factory utilization and inventory)
	 */
	 ...
	 * 
	 * @param report
	 *            the price report for previous day
	 */
