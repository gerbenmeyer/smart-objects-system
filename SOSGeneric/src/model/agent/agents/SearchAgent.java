...
public class SearchAgent extends Agent {
	...
	public void generateDetailsContent(HtmlDetailsContentGenerator detailsPane, HashMap<String, String> params) {
		...
		if (agents.size() > MAX_AGENTS) {
			detailsPane.add(HtmlTool.p(HtmlTool.img("warning.png", "Warning") + " Too many " + Settings.getProperty(Settings.KEYWORD_DEEPLINK)
					+ "s, only showing first " + MAX_AGENTS + "."));
		}
		...
		for (int i = 0; i < Math.min(agents.size(), MAX_AGENTS); i++) {
			...
			if (smallDetailsPane) {
				detailsPane.addDataRowLink(av.getIcon(), av.get(Agent.LABEL), statusIcon, av.getID() + ".map");
			} else {
				detailsPane.addDataRowLink(av.getIcon(), av.get(Agent.LABEL), av.get(Agent.DESCRIPTION), statusIcon, av.getID() + ".map");
			}
		}
