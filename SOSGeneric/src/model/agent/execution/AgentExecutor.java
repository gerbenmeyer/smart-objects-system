...
public class AgentExecutor extends Thread {
	 ...
	 * 
	 * @param delay in miliseconds
	 * @param DEBUG set to true if debug output must be provided
	 */
	public AgentExecutor (int delay, boolean DEBUG) {
		...
		if (DEBUG) System.out.println ("Processor: New Processor");
	}
	...
	/**
	 * @return wether this AgentExecutor is finished.
	 */
	...
	public boolean timeout (long timeOut) throws InterruptedException {
		...
		synchronized (this) {
			while (!done && !interrupted) {
				...
				if (delay <= 0) {
					if (DEBUG) System.out.println ("Processor: terminated");
					interrupt ();
					interrupted = true;
				} else {
					wait(delay);
	...
	public void run() {
		while (!interrupted) {
			if (agent != null && !done && !interrupted) {
				if (DEBUG) System.out.println ("Processor: " + agent.toString() + " > start.");
				try {
					agent.execute();
					synchronized (this) {
						if (DEBUG) System.out.println ("Processor: " + agent.toString () + " < completed.");
						done = true;
						notifyAll();
					}
		...
		if (DEBUG) System.out.println ("Processor: Interrupted.");
	}
}