 ...
 * Updated : $Date: 2005/06/09 16:35:03 $
 *           $Revision: 1.10 $
 */
...
public class BOMBundle implements Transportable, java.io.Serializable {
  ...
  public synchronized void addSegment(String name, int[] products) {
    ...
    if (segmentName == null) {
      ...
      segmentName = (String[])
	ArrayUtils.setSize(segmentName, segmentCount + 5);
      segmentProducts = (int[][])
	ArrayUtils.setSize(segmentProducts, segmentCount + 5);
    }
    segmentName[segmentCount] = name;
    segmentProducts[segmentCount] = products == null
      ? new int[0]
      : (int[]) products.clone();
    segmentCount++;
  }
  ...
  /**
   * Returns the products in the product segment at the specified
   * index (position).
   * @since 0.9.6
   */
  ...
  public void write(TransportWriter writer) {
    ...
    for (int i = 0, n = segmentCount; i < n; i++) {
      writer.node("segment")
	.attr("name", segmentName[i]);
      for (int j = 0, m = segmentProducts[i].length; j < m; j++) {
	writer.node("product").attr("id", segmentProducts[i][j])
	  .endNode("product");
      }
